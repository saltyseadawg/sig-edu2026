...
\maketitle
\begin{abstract}
In this paper we discuss the development of Kelvi\footnote{url redacted for anonymity}, an open source web-based dictionary and morphological parser designed to aid Tamil learners in developing their literacy skills. 
...
Overall there number around 80 million Tamil speakers, with nearly 240,000 speakers in Canada alone (\citealp{government_of_canada_language_2025}, \citealp{noauthor_tamil_2025}). 
...
See Figure (\ref{fig:gramble-display}), where a complete utterance is expressed as one word. \\
\begin{figure}[h]
  \includegraphics[width=\columnwidth]{kelvi-example.png}
  \caption{Tamil word as it appears in Kelvi}
...
The tool we built, Kelvi, is a web-based dictionary and morphological parser that helps the learner who comes 
...
morphological generator Gramble \citep{littell_gramble_2024} that was used in a pedagogical context for Oneida \citep{lu-etal-2024-empowering}. 
...
We built a medium fidelity prototype in Figma to test our design, accessible \href{https://www.figma.com/proto/1tpcdO49QkVfDOQYSrZ5YH/Tamil-Dict-Design-updated?node-id=5-288&starting-point-node-id=5%3A288&t=XY9IO1vT16vxJg0e-1}{here}. 
...
\begin{figure}[t]
  \includegraphics[width=\columnwidth]{tamil-fluency-literacy.png}
  \caption{Graph displaying participants' self-described fluency and literacy in Tamil.}
...
\begin{figure}[h]
  \includegraphics[width=\columnwidth]{graph-lookup.png}
  \caption{Graph of participants' methods for looking up Tamil words.}
...
%insert image
\begin{figure}[h]
  \includegraphics[width=\columnwidth]{graph-use-tool.png}
  \caption{Graph of participants' responses to if they would use a tool like the prototype.}
...
In this section we discuss the different components of Kelvi and how they interact. 
\begin{figure}[h]
  \includegraphics[width=\columnwidth]{system-diagram.png}
...
any suffix included in the Gramble JSON file (generation of this file is discussed in the following section). If there are multiple matches, 
...
If the “add-back” field is non-empty, we add this material 
...
matched against as described in the systems design (\ref{sec:suffix-matching}). 
...
\begin{figure}[t]
  \includegraphics[width=\columnwidth]{tamil-gramble.png}
...
user in all cases, as shown in Figure \ref{fig:gramble-display}. Finally, add-back is used as a 
secondary lemmatizer when the primary fails, as detailed above in section \ref{sec:suffix-matching}. 
...
\begin{figure}[t]
  \includegraphics[width=\columnwidth]{puthamkal.png}
...
\begin{table}
  \begin{tabular}{c|c|c}
    ...
    \hline
    Lemma not in dataset & 10 & 11\% \\
    Failed lemmatization & 79 & 89\% \\
    \hline
    Total & \textbf{89} & \\
  \end{tabular}
...
as shown in Table \ref{tab:no-result}. This delineation will help us isolate which types of 
...
equitable representation of language variation, and data privacy and consent. 
...
using language data and putting it behind a pay-wall, especially given one of the 
authors is a member of the language community the tool was built for. 
...
\begin{enumerate}
  \item Find the meaning of \foreignlanguage{tamil}{பெரியவர்களுக்கு} using the prototype interface. Try entering the word using Tamglish\footnote{Colloquial term used to refer to the code-mixing of Tamil and English in casual speech.} / English letters.
  \item Find the meaning of \foreignlanguage{tamil}{அறிந்துகொண்டேன்} but type the word in Tamil.
